...
async def boss_kill(message, boss_name, input_time_str=None):
    ...
    boss_list[boss_name]['next_spawn_time'] = regen_time
...
async def print_boss_list(message):
    ...
    for boss in sorted(boss_list.values(), key=lambda x: x['next_spawn_time'] if x['next_spawn_time'] else datetime.datetime.max):
        next_spawn_time_str = "미정"
        if boss['next_spawn_time']:
            next_spawn_time_str = boss['next_spawn_time'].strftime("%m/%d %H:%M")
        boss_list_str += f"{boss['name']} (Lv. {boss['level']}) => 예상 출현 시간: {next_spawn_time_str}\n"
    boss_list_str += "```"
